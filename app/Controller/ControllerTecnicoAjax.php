...
class ControllerTecnicoAjax extends ClassTecnico
{
    ...
    public function recuperarVar()
    {
        ...
        if (isset($_POST['rg'])) {
            $this->rg = filter_input(INPUT_POST, 'rg', FILTER_SANITIZE_STRING);
        }
        if (isset($_POST['cpf'])) {
            $this->cpf = filter_input(INPUT_POST, 'cpf', FILTER_SANITIZE_STRING);
        }
        ...
        if (isset($_POST['ativo'])) {
            $this->ativo = filter_input(INPUT_POST, 'ativo', FILTER_SANITIZE_STRING);
        }
    }
    ...
    public function listarAtivos()
    {
        ...
        if ($itens < 1) {
            ...
            $rowTecnicos = $this->listarTecnicosAtivos($inicio, $registros);
        ...
        <tbody>';
            ...
            foreach ($rowTecnicos as $key => $tecnico) {
                ...
                '<tr>
                    ...
                    <td>
                        <button id="info" class="btn btn-info shadow" value="' . $tecnico['id'] . '" data-toggle="modal" data-target="#modal-info"><i class="fa fa-info"></i></button>                
                        <button id="lock" class="btn btn-warning shadow" value="' . $tecnico['id'] . '"><i class="fa fa-lock"></i></button>
                    </td>
                </tr>';
            }
            echo '</tbody>
    ...
    public function listarInativos()
    {
        ...
        $consultaCompleta = $this->listarTecnicosInativosCompleto();
        if ($consultaCompleta != null) {
            $itens = count($consultaCompleta);
        } else {
            $itens = 0;
        ...
        if ($itens < 1) {
            ...
            $rowTecnicos = $this->listarTecnicosInativos($inicio, $registros);
            echo '<p>' . $itens . ' Resultados para essa consulta</p>';
            echo
            '<div class="table-rep-plugin">
                <div class="table-responsive" data-pattern="priority-columns">
            ...
            foreach ($rowTecnicos as $key => $tecnico) {               
                ...
                echo
                    '<tr>
                        ...
                        <td>' . $tecnico['cidade'] . '</td> 
                        <td>                
                            <button id="unlock" class="btn btn-warning shadow" value="' . $tecnico['id'] . '"><i class="fas fa-unlock"></i></button>                
                            <button id="del" class="btn btn-danger shadow" value="' . $tecnico['id'] . '"><i class="fa fa-trash text-light"></i></button>                
                        </td>
                    </tr>';
            }
            echo '</tbody>
    ...
    public function cadastro()
    {
        ...
        $consulta = $this->consultaCpf($this->cpf);

        if ($consulta == null) {
            $create = $this->createTecnico($this->nome, $this->rg, $this->cpf, $this->telefone, $this->cidade, $this->bairro, $this->logradouro, $this->complemento, $this->uf, $this->cep, $this->complemento);
        } else {
            echo "Já existe um técnico com esse CPF.";
        }
